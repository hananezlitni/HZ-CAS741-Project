...
Plan \wss{put your library name in the title}} 
...
the Library of Simplex Method Solvers (\famname{}) tool. \wss{I recently
  modified the blank project template to put the (equivalent of) the famname
  command in a common file, which can be shared between all your files.  You
  might want to do the same.}  It is based on the tool's commonality analysis
...
\wss{You should reference your SRS here.}
...
Spencer Smith \wss{\LaTeX{} has a rule that it inserts two spaces at the end of
  a sentence.  It detects a sentence as a period followed by a capital letter.
  This comes up, for instance, with Dr. Smith.  Since the period after Dr.\
  isn't actually the end of a sentence, you need to tell \LaTeX{} to insert one
  space.  You do this either by Dr.\ Smith (if you don't mind a line-break
  between Dr.\ and Smith), or Dr.~Spencer Smith (to force \LaTeX{} to not insert
  a line break).} and my CAS 741 classmates which comes from formal reviews of
the document.
...
the comments, I apply them one by one and make changes in the document. \wss{You
  could be more specific (using Repos.xlsx) on who exactly is going to review
  your documentation.}
...
\wss{It would be nice to have a blurb here to explain why the subsections below
  cover the requirements.  References to the SRS would be good.  In this case,
  you could reference any table of data constraints.}

\begin{enumerate}
	\item{\textbf{T6: No Objective Function}} \label{TestNoObjcFunc}
...
\begin{enumerate}
	...
	How test will be performed: Asking participants to try the tool \wss{you
          should clarify that you are going to make a tool using your library}
...
\begin{enumerate}
	...
	number of inputs and observe how it would behave \wss{You need to be
          more specific.}
